...
public extension HTTPClient { 
    ...
    func handlingDataTask<T: Decodable>(
        ...
        case 200...299:
            ...
            if let onlineSimError = data?.decode(model: OnlineSimException.self), onlineSimError.hasError {
                return .failure(.request(localizedDiscription: onlineSimError.description))
            } else if let decodeData = data?.decode(model: responseModel) {
                return .success(decodeData)
            } else if let data, responseModel is String.Type {
                let response = String(decoding: data, as: UTF8.self)
                let error = OnlineSimError.allCases.first { $0.rawValue == response }
                if let error {
